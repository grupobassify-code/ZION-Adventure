...
export const TouchControls: React.FC<TouchControlsProps> = ({
  ...
  if (isPortrait) {
    return (
      <div 
        ...
        className="relative w-full flex-1 min-h-[220px] max-h-[380px] p-2.5 sm:p-4 flex flex-col justify-between z-20 select-none touch-control-surface bg-gradient-to-b from-[#050711] via-[#090e24] to-[#040610] border-t border-cyan-900/40"
      >
        {/* Secondary Row / Utilities */}
        <div className="flex items-center justify-between gap-2 px-1">
          {/* Mode Switcher */}
          <div className="flex items-center gap-1.5 bg-slate-950/80 px-2.5 py-1 rounded-full border border-slate-800 text-[10px] text-cyan-400 font-mono">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className={`px-3 py-1.5 rounded-xl flex items-center gap-1 text-xs font-bold border transition-transform active:scale-95 ${
                inputs.dash
            ...
            >
              ...
              <span className="text-[10px] font-mono">DASH</span>
            </button>
            ...
            >
              ...
              <span className="text-[10px] font-mono">ESCUDO</span>
            </button>
            ...
            >
              ...
              <span className="text-[10px] font-mono">SP {energy}</span>
            </button>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between px-1 sm:px-2 pt-2">
          ...
          <div className="flex flex-col items-center">
            {controlMode === 'joystick' ? (
              <div
                ...
                className="relative w-28 h-28 sm:w-32 sm:h-32 rounded-full bg-slate-950/80 backdrop-blur-md border-2 border-cyan-500/40 shadow-[0_0_20px_rgba(6,182,212,0.25)] flex items-center justify-center cursor-grab active:cursor-grabbing touch-control-surface"
              >
                ...
                >
                  <div className="w-3.5 h-3.5 rounded-full bg-cyan-200/80" />
                </div>
              </div>
            ) : (
              <div className="flex items-center gap-2 bg-slate-950/70 p-2 rounded-2xl border border-slate-800 shadow-xl touch-control-surface">
                <button
                  onPointerDown={(e) => {
                    e.preventDefault();
                    onUpdateInput('left', true);
                  ...
                  className={`w-13 h-13 rounded-xl flex items-center justify-center font-bold text-lg border-2 ${
                    inputs.left
          ...
          <div className="flex items-center gap-2 touch-control-surface">
            ...
            <button
              ...
              className={`w-12 h-12 rounded-2xl flex flex-col items-center justify-center font-bold border-2 transition-transform active:scale-90 ${
                inputs.dagger
            ...
            <button
              ...
              className={`w-13 h-13 rounded-2xl flex flex-col items-center justify-center font-bold border-2 transition-transform active:scale-90 ${
                inputs.attack
            ...
            <button
              ...
              className={`w-14 h-14 rounded-2xl flex flex-col items-center justify-center font-bold border-2 transition-transform active:scale-90 ${
                inputs.jump
            ...
            >
              <span className="text-xs font-black tracking-wider">SALTAR</span>
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col items-start gap-1 pointer-events-auto p-2">
        ...
        {controlMode === 'joystick' ? (
          ...
          <div
            ...
            onPointerUp={handlePointerUpJoystick}
            onPointerCancel={handlePointerUpJoystick}
            className="relative w-26 h-26 sm:w-30 sm:h-30 rounded-full bg-slate-950/80 backdrop-blur-md border-2 border-cyan-500/40 shadow-[0_0_20px_rgba(6,182,212,0.25)] flex items-center justify-center cursor-grab active:cursor-grabbing"
          >
            <div className="absolute w-full h-[1px] bg-cyan-500/20" />
            <div className="absolute h-full w-[1px] bg-cyan-500/20" />
            <div className="absolute w-10 h-10 rounded-full border border-cyan-500/30" />
            ...
            <div
              ...
              className={`w-11 h-11 sm:w-13 sm:h-13 rounded-full flex items-center justify-center shadow-lg border-2 ${
                isDraggingJoystick
                  ? 'bg-gradient-to-br from-cyan-400 to-blue-600 border-cyan-200 text-slate-950 shadow-[0_0_15px_rgba(34,211,238,0.8)]'
                  : 'bg-slate-800/90 border-cyan-500/60 text-cyan-300'
              }`}
          ...
          <div className="flex items-center gap-1.5 bg-slate-950/70 backdrop-blur-md p-1.5 rounded-2xl border border-slate-800 shadow-xl">
            <button
              ...
              onPointerUp={() => onUpdateInput('left', false)}
              onPointerCancel={() => onUpdateInput('left', false)}
              className={`w-12 h-12 rounded-xl flex items-center justify-center font-bold text-base border-2 ${
                inputs.left
                  ? 'bg-cyan-500 text-slate-950 border-cyan-200 shadow-[0_0_15px_rgba(6,182,212,0.6)]'
                  : 'bg-slate-900/80 text-cyan-300 border-cyan-500/40'
              }`}
      ...
      <div className="flex flex-col items-end gap-1.5 pointer-events-auto p-2">
        {/* Secondary Row: Dash, Block, Special */}
        <div className="flex items-center gap-1.5 bg-slate-950/70 backdrop-blur-md p-1 rounded-2xl border border-slate-800 shadow-lg">
          <button
            onPointerDown={(e) => {
              e.preventDefault();
              onUpdateInput('dash', true);
            ...
            onPointerUp={() => onUpdateInput('dash', false)}
            onPointerCancel={() => onUpdateInput('dash', false)}
            className={`w-10 h-10 rounded-xl flex flex-col items-center justify-center text-xs font-bold border transition-transform active:scale-95 ${
              inputs.dash
                ? 'bg-sky-400 text-slate-950 border-sky-200 shadow-[0_0_12px_rgba(56,189,248,0.8)]'
                : 'bg-slate-900/90 text-sky-300 border-sky-500/40'
            }`}
        ...
        <div className="flex items-center gap-1.5 bg-slate-950/80 backdrop-blur-md p-1.5 rounded-3xl border border-slate-800 shadow-2xl">
          <button
            onPointerDown={(e) => {
              e.preventDefault();
              onUpdateInput('dagger', true);
