      ...
      <header className="relative z-10 w-full max-w-5xl flex items-center justify-between gap-2 pt-0.5 sm:pt-1">
        <div className="flex items-center gap-1.5 sm:gap-2">
          {view !== 'title' && (
            <button
              ...
              className="flex items-center gap-1 sm:gap-1.5 px-2.5 py-1 sm:px-3 sm:py-1.5 rounded-lg bg-slate-800/80 hover:bg-slate-700 border border-cyan-500/30 text-cyan-300 font-bold text-[10px] sm:text-xs shadow-lg transition-all active:scale-95"
            >
          ...
          <div className="flex items-center gap-2">
            <span className="px-2 py-0.5 rounded-full bg-cyan-500/20 border border-cyan-400/40 text-cyan-300 text-[9px] sm:text-[10px] font-mono font-black tracking-wider uppercase truncate max-w-[130px] sm:max-w-none">
              {t('version')}
            </span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-1 sm:gap-2">
          ...
          >
            ...
            <span className="hidden md:inline">{t('jukebox')}</span>
          </button>
          ...
          >
            ...
            <span className="hidden md:inline">{t('controls')}</span>
          </button>

          <button
            onClick={onToggleAudio}
            className="p-1.5 sm:p-2 rounded-xl bg-slate-900/80 hover:bg-slate-800 border border-slate-700 text-slate-300 transition-all"
            title={t('audioToggleTitle')}
          >
          ...
          <div className="w-full flex flex-col sm:flex-row items-center justify-between gap-3 mb-5 border-b border-slate-800 pb-3">
            <div>
              <div className="flex items-center gap-2 text-xs font-mono text-cyan-400">
                <Layers className="w-3.5 h-3.5" />
                <span>{t('saveFileLabel', { name: activeSlot?.name || 'Zion' })}</span>
              </div>
                      ...
                      <div>
                        <div className="flex items-center gap-2 flex-wrap">
                          <span className={`px-2.5 py-0.5 rounded text-[10px] font-mono font-black tracking-wider uppercase border ${
                            ...
                            {clockDone ? t('clockRestored') : t('clockFractured')}
                          </span>
                      ...
                      {onlyUpUnlocked ? (
                        <>
                          <span className="text-xs font-mono font-bold text-orange-400 flex items-center gap-1">
                            <Play className="w-3.5 h-3.5 fill-orange-400" /> {t('modeUnlocked')}
                          </span>
                          <button
                            id="play-only-up-btn"
                            type="button"
                        ...
                        <div className="w-full flex items-center justify-between text-xs text-slate-500 font-mono">
                          <span className="flex items-center gap-1">
                            <Lock className="w-3.5 h-3.5" /> {t('beatNeonForest')}
                          </span>
                          <span className="text-slate-600">{t('lockedStatus')}</span>
                        </div>
                      )}
                    </div>
                  </div>
                      ...
                      {specialUnlocked ? (
                        <>
                          <span className="text-xs font-mono font-bold text-purple-400 flex items-center gap-1">
                            <Play className="w-3.5 h-3.5 fill-purple-400" /> {t('specialStageActive')}
                          </span>
                          <button
                        ...
                        <div className="w-full flex items-center justify-between text-xs text-slate-500 font-mono">
                          <span className="flex items-center gap-1">
                            <Lock className="w-3.5 h-3.5" /> {t('beatOneSpecial')}
                          </span>
                          <span className="text-slate-600">{t('lockedStatus')}</span>
                        </div>
                      )}
                  ...
                  >
                    ...
                    <div className="p-4 pt-3 border-t border-slate-800/80 bg-slate-950/40 flex items-center justify-between">
                      {vsAiUnlocked ? (
                        <>
                          <span className="text-xs font-mono font-bold text-emerald-400 flex items-center gap-1">
                            <Play className="w-3.5 h-3.5 fill-emerald-400" /> {t('modeUnlocked')}
                          </span>
                          <button
                            id="play-vs-ia-btn"
                            type="button"
                        ...
                        <div className="w-full flex items-center justify-between text-xs text-slate-500 font-mono">
                          <span className="flex items-center gap-1">
                            <Lock className="w-3.5 h-3.5" /> {t('beatSakuraBoss')}
                          </span>
                          <span className="text-slate-600">{t('lockedStatus')}</span>
                        </div>
                      )}
                      ...
                      {timeAttackUnlocked ? (
                        <>
                          <span className="text-xs font-mono font-bold text-amber-400 flex items-center gap-1">
                            <Play className="w-3.5 h-3.5 fill-amber-400" /> {t('modeUnlocked')}
                          </span>
                          <button
                        ...
                        <div className="w-full flex items-center justify-between text-xs text-slate-500 font-mono">
                          <span className="flex items-center gap-1">
                            <Lock className="w-3.5 h-3.5" /> {t('beatLavaBoss')}
                          </span>
                          <span className="text-slate-600">{t('lockedStatus')}</span>
                        </div>
                      )}
          ...
          <div className="w-full flex items-center justify-between gap-3 mb-5 border-b border-slate-800 pb-3">
            <div>
              <div className="text-xs font-mono text-cyan-400 uppercase">
                {t('zonePrefix', { name: getZoneName(selectedZone) })}
              </div>
              <h2 className="text-2xl sm:text-3xl font-black text-white font-heading">
                {t('selectActTitle')}
              </h2>
                    ...
                    <div className="mt-4 pt-3 border-t border-slate-800 flex items-center justify-between">
                      {isUnlocked ? (
                        ...
                        >
                          ...
                          <span>{isCompleted ? t('repeatActBtn') : t('playActBtn')}</span>
                        </button>
                      ) : (
          ...
          <div className="w-full bg-slate-900/95 border-2 border-cyan-500/40 rounded-2xl p-6 shadow-2xl">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
              <div className="bg-slate-950/70 p-3 rounded-xl border border-slate-800 flex items-center justify-between">
                <div>
                  <div className="font-bold text-white">{t('ctrlMovement')}</div>
                  <div className="text-slate-400 text-[11px]">{t('ctrlMovementDesc')}</div>
                </div>
              ...
              <div className="bg-slate-950/70 p-3 rounded-xl border border-slate-800 flex items-center justify-between">
                <div>
                  <div className="font-bold text-white">{t('ctrlSword')}</div>
                  <div className="text-slate-400 text-[11px]">{t('ctrlSwordDesc')}</div>
                </div>
                <div className="px-2 py-1 rounded bg-slate-800 text-cyan-300 font-mono font-bold">
                  {t('ctrlSwordKeys')}
                </div>
              ...
              <div className="bg-slate-950/70 p-3 rounded-xl border border-slate-800 flex items-center justify-between">
                <div>
                  <div className="font-bold text-white">{t('ctrlDagger')}</div>
                  <div className="text-slate-400 text-[11px]">{t('ctrlDaggerDesc')}</div>
                </div>
                <div className="px-2 py-1 rounded bg-slate-800 text-cyan-300 font-mono font-bold">
                  {t('ctrlDaggerKeys')}
                </div>
              ...
              <div className="bg-slate-950/70 p-3 rounded-xl border border-slate-800 flex items-center justify-between">
                <div>
                  <div className="font-bold text-white">{t('ctrlShield')}</div>
                  <div className="text-slate-400 text-[11px]">{t('ctrlShieldDesc')}</div>
                </div>
                <div className="px-2 py-1 rounded bg-slate-800 text-cyan-300 font-mono font-bold">
                  {t('ctrlShieldKeys')}
                </div>
              ...
              <div className="bg-slate-950/70 p-3 rounded-xl border border-slate-800 flex items-center justify-between">
                <div>
                  <div className="font-bold text-white">{t('ctrlDash')}</div>
                  <div className="text-slate-400 text-[11px]">{t('ctrlDashDesc')}</div>
                </div>
                <div className="px-2 py-1 rounded bg-slate-800 text-cyan-300 font-mono font-bold">
                  {t('ctrlDashKeys')}
                </div>
              ...
              <div className="bg-slate-950/70 p-3 rounded-xl border border-slate-800 flex items-center justify-between">
                <div>
                  <div className="font-bold text-white">{t('ctrlSpecial')}</div>
                  <div className="text-slate-400 text-[11px]">{t('ctrlSpecialDesc')}</div>
                </div>
                <div className="px-2 py-1 rounded bg-slate-800 text-cyan-300 font-mono font-bold">
                  {t('ctrlSpecialKeys')}
                </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm">
          ...
          >
            <h3 className="text-lg font-black text-white font-heading mb-2">
              {t('newSlotModalTitle', { num: newSlotModal.slotId + 1 })}
            </h3>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm">
          <div className="w-full max-w-sm bg-slate-900 border-2 border-red-500/50 rounded-2xl p-5 shadow-2xl text-center">
            ...
            <h3 className="text-lg font-black text-white font-heading mb-1">
              {t('deleteSlotModalTitle', { num: deleteConfirmModal.slotId + 1 })}
            </h3>
            <p className="text-xs text-slate-400 mb-4">
              {t('deleteSlotModalDesc')}
            </p>
